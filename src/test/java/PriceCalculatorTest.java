...
class PriceCalculatorTest {
    ...
    void calculatePrice() {
        ...
        BigDecimal full_price = new BigDecimal(test_props.getProperty(Props.LP_L_PRICE));
        BigDecimal month_limit = new BigDecimal(test_props.getProperty(Props.TOT_MONTH_DISC_AMT));


        TrxLine tl = new TrxLine("2018-04-29 L LP");
        pc.calculatePrice(tl);
        assertEquals("1 st. L LP in month price before discount", full_price, tl.getPrice());
        assertEquals("1 st. L LP in month discount", new BigDecimal("0"), tl.getDiscount());

        tl = new TrxLine("2018-04-29 L LP");
        pc.calculatePrice(tl);
        assertEquals("2 nd. L LP in month price before discount", full_price, tl.getPrice());
        assertEquals("2 nd. L LP in month discount", month_limit, tl.getDiscount());
    }
}