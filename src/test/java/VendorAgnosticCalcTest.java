...
class VendorAgnosticCalcTest {
    ...
    void applyOneSPriceRule() {
        ...
        BigDecimal min_s_price = new BigDecimal(test_props.getProperty(Props.MR_S_PRICE));
        BigDecimal lp_s_price = new BigDecimal(test_props.getProperty(Props.LP_S_PRICE));

        TrxLine tl = new TrxLine("2018-04-29 S LP");
        tl.setPrice(lp_s_price);
        va_calc.applyOneSPriceRule(tl);
        assertEquals("LP S discount", new BigDecimal("4"), tl.getDiscount());

        tl = new TrxLine("2018-04-29 S MR");
        tl.setPrice(min_s_price);
        va_calc.applyOneSPriceRule(tl);
        assertEquals("MR S no discount", new BigDecimal("0"), tl.getDiscount());
    }
    ...
    void applyMonthDiscountLimitRule() {
        ...
        assertEquals("Discount bellow limit in 2018-04", new BigDecimal("5"), tl.getDiscount());
        ...
        va_calc.applyMonthDiscountLimitRule(tl);
        assertEquals("Discount hit limit in 2018-04", new BigDecimal("0.55"), tl.getDiscount());
        ...
        va_calc.applyMonthDiscountLimitRule(tl);
        assertEquals("No more discounts for 2018-04", new BigDecimal("0.00"), tl.getDiscount());
        ...
        va_calc.applyMonthDiscountLimitRule(tl);
        assertEquals("Discount bellow limit in 2018-05", new BigDecimal("5"), tl.getDiscount());
    }
}